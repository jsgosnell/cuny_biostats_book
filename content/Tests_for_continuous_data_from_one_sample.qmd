...
two-tailed tests since the distribution is symmetric.
...
for(i in 1:number_of_simulations){
sampling_experiment[i,k] = mean(rnorm(as.numeric(sample_size[k])))
}
}

library(reshape2)
sampling_experiment_long <- melt(sampling_experiment, variable.name = "Sample_size", value.name = "mean")
sampling_experiment_long$Sample_size <- paste("Sample size of ", sampling_experiment_long$Sample_size)

ggplot(sampling_experiment_long,aes(x=mean)) +
  geom_histogram(color="black") +
  labs(title="Observed means from 1000 random draws using varying sample sizes from a normal distribution",
       x= "Mean",
       y= "Frequency")+
