...
just counts those below the proposed mean. In theory, approximately half
the values should be under the proposed mean if H~O~ is true. The
proportion below is compared to .5 using a binomial test.
...
We can demonstate this with an imaginary population (and also
...
Now's let's samples of 50 from it a lot of times
(`r number_of_simulatons` here) and plot the means of these samples.
...
```{r}
actual_sample <- sample(population_unif$value,50, replace = F)
```

Now (here's the odd part): Let's sample from that sample with
replacement to get "new" samples, then get those means, again,
`r number_of_simulation` times.

```{r}
boostrap_outcomes <- data.frame(mean = rep(NA, number_of_simulations), sd = NA)
for (i in 1:number_of_simulations){
example_bootstrap <-sample(actual_sample, length(actual_sample), replace = T)
boostrap_outcomes$mean[i] <- mean(example_bootstrap)
boostrap_outcomes$sd[i] <- sd(example_bootstrap)
}
```

When we plot them we see it looks very much like our original
distribution!
...
Let's demonstrate this approach with our iris data. First, we make "new"
datasets from our original data by sampling (with replacement) samples
of the same size from our original dataset.

```{r}
boostrap_data<- iris[iris$Species == "virginica","Sepal.Length"]
boostrap_outcomes <- data.frame(mean = rep(NA, number_of_simulations), sd = NA)
for (i in 1:number_of_simulations){
...
normal distribution. We can also carry this out using the *boot*
...
and then use the *boot.ci* function to find a 95% confidence interval.
...
Although p-values are sometimes connected to these methods, we won't
develop those here. Just focus on getting the confidence interval for
your signal using bootstrap.
