...
`{r} chisq.test(matrix(c(9,4,9,7), 2, 2, byrow=T))}`
...
`{r} library(reshape2) birds_wide <- dcast(birds_original, Age~Species) birds_wide}`

We can make the matrix with wide data

`{r} chisq.test(matrix(c(birds_wide$Junco, birds_wide$Finch), nrow = 2))}`
...
`{r} chisq.test(cbind(birds_wide$Finch, birds_wide$Junco))}`
