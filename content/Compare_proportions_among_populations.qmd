...
mean the parameter is different for every group. Make sure you process
the difference.
...
the male/female ratio is catbirds. But we need to test that.

In general, after obtaining a significant p-value from a
multi-population question, we follow up with **post-hoc** specific
comparisons. There are many ways to do this. For example, you can do
compare all possible pairs or just certain combinations. Why would this
matter?
...
We will first use a very simple method that is no longer recommended but
is useful as a starting point. One options is to control the FWER by
dividing $\alpha$ by the number of post-hoc tests we intend to run. For
the above example, if we do all pairs comparisons we would be running 10
comparison (4+3+2+1...). So instead of using .05 as a cutoff, we would
use .005.
...
Note the unadjusted p-value for this comparison pair is the same we
...
```{library(rcompanion)}
bonf_correct <- pairwiseNominalIndependence(bird_ratio, method = "bonf")
