...
+--------------+--------------+--------------+--------------+
| **Observed** | **Cardinal** | **Junco**    | Totals       |
+--------------+--------------+--------------+--------------+
| **Old**      | 9            | 4            | 13           |
+--------------+--------------+--------------+--------------+
| **Young**    | 9            | 7            | 16           |
...
```{r}
birds <- data.frame(Old = c("Y","N","Y", "N"),
                      Bird = c("Junco", "Junco", "Finch", "Finch"),
                      Number = c(4, 7, 9, 9))
#mosaic plot####
#
ggplot(birds, aes(x= Bird, y = Number)) +
  geom_col(aes(fill = Old)) + 
  xlab("Species") +
  ylab("Frequency") +
...
```{r}
ggplot(birds, aes(x= Bird, y = Number)) +
  geom_col(aes(fill = Old), position = "fill")
...
| data)**      |              |              |
+--------------+--------------+--------------+
| **Reject     | Type I error | Power (1-    |
| H~O~**       | (P\[ɑ\])     | $\beta$ )    |
+--------------+--------------+--------------+
| **Do not     | Correct      | Type II      |
| reject       | (P\[1- ɑ\])  | Error ( Pr\  |
| H~O~**       |              | \[$\beta$\]) |
+--------------+--------------+--------------+
