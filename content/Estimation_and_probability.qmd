...
ggplot(sample_outcomes_2, aes(mean)) + 
  ...
  geom_vline(aes(xintercept=mean(mean)+
                     2*sd(mean), color="2 standard deviations above")) +
        scale_color_manual(values = colors)+
   annotate("text", label = "mean", y = 150, x = mean(sample_outcomes$mean), color = "black") +
...
time. We call this range a 95% confidence interval. For example,
...
sizes over ten 2 is a good estimate. We could also have other numbers.
For example, we could have a 90% confidence interval.
