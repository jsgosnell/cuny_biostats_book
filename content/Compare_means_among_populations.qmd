...
Our first approach uses a new (to us) method known as Tukey's method,
Tukey's Honestly Significant Difference/HSD, Tukey-Kramer method, and
many other names. It focuses on all possible pair-wise comparisons and
controls for the FWER using a studentized range approach (not fully
developed here, but similar to z-transform but focused on the range of
means and using the estimated standard deviation; similar to t-statistic
in this aspect and also developed by Student). Using this approach we
see that all species differ significantly from all others; the output
also provides estimates of the differences, which match up with our
...
(columns are not independent). However, if we instead compared
