...
```{r}
iris$is_versicolor <- iris$Species
iris$is_versicolor <- revalue(iris$is_versicolor, c("versicolor"="yes",
                                                    "setosa"="no",
                                                    "virginica"="no"))
```
...
and the approach we used for single-sample t-tests. This is because the
t-distribution is a special case of the F distribution.

To prove this, let's only consider two species.

```{r}
two_species_subset <- iris[iris$Species!=]
```
