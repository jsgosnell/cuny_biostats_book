...
The average length of fish in this population is `r mean(lengths)` cm. We can
then simulate a sample from this population. In fact, let's simulate 2 and
compare the means of each.
...
The mean length for fish in sample 1 is `r mean(sample_1)` cm, while that in
sample 2 is `r mean(sample_2)` cm (Note: if you view this on the webpage you
will see a number, but in the actual qmd file you see R code here - this is an
example of merging code and text!). These are both close to the true value, but
they are also both slightly different - this is sampling error!
