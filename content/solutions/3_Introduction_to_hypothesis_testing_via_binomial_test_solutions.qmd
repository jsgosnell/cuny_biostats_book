...
-   Calculate and compare p-values obtained using
    ...
    *Note we can calculate a p-value using the simulated distribution,
    the actual distribution (which is exact in this case), and the test
    (which is usign the actual distribution!).*
...
students helped the dog.I found a p-value of .04, so I reject the null hypothesis that the
...
or a field of interest that that includes a binomial test
(hint: use Google Scholar or another search engine to search for
a keyword of interest plus "binomial test" in quotes). Briefly answer the following
questions

-   What is the name of the paper and who are the authors?
-   What is the main finding related to the test?
