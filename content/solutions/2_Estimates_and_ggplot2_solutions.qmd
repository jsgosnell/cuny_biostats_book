...
uses the theme_bw.  For more, see
<https://ggplot2.tidyverse.org/reference/ggtheme.html>{target="_blank"}.  You can also save and name a
...
ggplot(sleep
       , aes(x=TotalSleep)) +
  geom_histogram() +
  labs (x ="Total sleep (hours per day",
        title = "Variation in sleep levels")
```
...
ggplot(sleep_by_predation
  ...
  labs(y="Total sleep \n (hours per day", 
       title="Sleep across different predation levels")
...
The call below find the file on the website and sources it.  **Source** runs the entire code at once (similar to
knitting an Rmd file or rendering a Qmd file) without showing any console output, but graphs and
objects are still produced!

You can also source files directly in Rstudio.  The
**Source** button is next to the **Run** command we've been using for
lines or segments.  
