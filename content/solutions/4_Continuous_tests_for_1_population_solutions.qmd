...
```{r}
sport <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vTFssjfGZ9w8Ng1qWVO4tRSZ4Cp2LpMeTVmler4_mcRhSk6932yLXi2b1a0V1b1tJlZ7rYpIhlgJ15f/pub?gid=1591196674&single=true&output=csv", header = T, stringsAsFactors = T)
library(BSDA)
...
obtained from such nests and they yield 25 females. Test the
...
```{r}
sport <- read.table("https://raw.githubusercontent.com/jsgosnell/CUNY-BioStats/refs/heads/master/datasets/ais.txt", header = T, stringsAsFactors = T)
```
...
or a field of interest that  includes a test we introduced in this module (t-test
is usually a good and common one to consider) (hint: use Google Scholar or another search engine to search for
a keyword of interest plus "t-test"). Briefly answer the following
questions

-   What is the name of the paper and who are the authors?
-   What trait is the test focused on?
