...
Once we have some data, the next step is often to summarize it. In fact, we've already done that in some ways. Some statistics like the mean may be considered a summary of the data. This may be useful because we prefer large datasets #remember sapling error!#(good sampling!) #remember to account for sampling error#, but making sense of a list of numbers can be really hard! Summaries help us describe, and eventually compare, datasets, which we are using to infer something about a population. (will learn more about family-wise error rate (FWER)later in the semester, reference **Compare proportions among groups**)
