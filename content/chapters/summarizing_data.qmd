...
eventually call these a t-test **(for two groups)** or ANOVA**(for more than 2 groups)** (when the trait we measure is
categorical) or a $\chi^2$ test (when the trait is categorical). Either
...
eventually analyze this type of data using **linear**regression or correlation.
