...
knowledge. The order of broken assumptions here reflects that found in
@gelman2006 but also common practice. Ignored here is their first main
point - the model must be valid! As noted in the multiple regression
chapter, we only should investigate relationships we have a mechanism to
explain. We will return to this point multiple times
...
connected to other issues, namely linearity. Many datasets will violate
both simultaneously, so addressing this issue is often best resolved by
...
assumptions. This approach was often used in the past. For example,
Box-Cox transformation helped researchers find the best way to transform
data to reduce issues with the distribution of residuals; this method
also tended to impact linearity and differences in variances.
