...
We can specify other data distributions using *generalized linear
models* (which are different than general linear models, which is
another term used to describe our "regular" linear models). These models
are also known as glm or glim (we'll use the glm jargon here). These
models let us consider data that do not fit a normal approximation.
@bolker2008 has an excellent review of when various distributions should
be used. The table below gives some of the more common distributions and
...
the inverse, so to determine the impact of a given coefficient on the
raw data we would combine the intercept and coefficients in this way.
...
If this occurs, some variables may need to be removed from
consideration.
...
experiment <- data.frame(                      predator_cue = factor(c(rep(c(rep("none",25),
                         ...
                         growth = c(rnorm(25,2),
                                    rnorm(25, 1.5),
                                    rnorm(25, .7)),
                         container = c(rep(letters[1:3],each=25)))
...
its own space (and received cues from a unique predator), you hopefully
recognize this as a lm/ANOVA that you know(almost) how to analyze. If
you need a reminder, see the [chapter on
ANOVAs.](../chapters/Compare_means_among_populations.qmd){target="_blank"}
...
This approach takes a lot of space (and time!). If I have a large tank, I may 
instead decide to put a few more oysters in there.  At an extreme, let's instead 
use one tank was used for each predator level, with twenty-five oysters
measured in each tank.However, this means our measurements are
connected; remember [blocking and paired
...
averaged_experiment <- summarySE(experiment, measurevar = "growth",
                             groupvars = c("predator_cue", "container"))
library(rmarkdown)
...
```{r, error=T}
average_analysis <- lm(growth~predator_cue, averaged_experiment)
Anova(average_analysis, type = "III")
```

but that leads to an issue! Since we only get 3 average outcomes and our
model requires 4 degrees of freedom (consider why), we are left with no
"noise" to make the denominator for our F ratio! This is partially due to a lack 
of replication - we'll fix that below after we explore another problem.  
...
```{r, error=T}
blocking_analysis <- lm(growth~predator_cue+container, experiment)
...
though the output is confusing. In general, the issue here is each container only 
measures one level of predator risk, so risk and container are completely 
confounded. This is (again) why we need replication in studies!  This was just a bad 
experiment... let's fix it now....


```{r}
set.seed(19)
experiment2 <- data.frame(                      predator_cue = factor(c(rep(c(rep("none",175),
                                               rep("low",175),
                                               rep("high",175)),1))),
                         growth = c(rnorm(175,2),
                                    rnorm(175, 1.5),
                                    rnorm(175, .7)),
                         container = c(rep(letters[4:24],each=25)))
experiment2[experiment2$growth<0, "growth"] <- 0

experiment_new <- merge (experiment, experiment2, all=T)
```

```{r, error=T}
blocking_analysis <- lm(growth~predator_cue+container, experiment_new)
Anova(blocking_analysis, type = "III")
```

Even with replicates we still get an issue...why?

```{r}
alias(blocking_analysis)
```

It's because the container variable can fully "explain" the predation levels. 
This is because each container only contributes to one level of other traits.
If instead each unit contributed to multiple levels, like in our
[feather experiment](../chapters/More_ANOVAs.qmd){target="blank"}) this
isn't an issue.

Consider instead if we had truly independent samples - the data might look 
like this:

```{r}
set.seed(19)
experiment3 <- data.frame(                      predator_cue = factor(c(rep(c(rep("none",200),
                                               rep("low",200),
                                               rep("high",200)),1))),
                         growth = c(rnorm(200,2),
                                    rnorm(200, 1.5),
                                    rnorm(200, .7)),
                         container = paste ("cont", 1:600, sep="_"))
experiment3[experiment3$growth<0, "growth"] <- 0
```

which is just our data but in unique containers.  Then we could analyze it like
this...

```{r}
unique_analysis <- lm(growth~predator_cue, experiment3)
Anova(unique_analysis, type = "III")
```
...
measured as simply a potential samples from a larger population. Using
that background, we use the information from the units to consider the
...
levels of the unit variable.



This is because we are using the means to
