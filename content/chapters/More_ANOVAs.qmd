...
One of the last chapters practice problems focused on bird feathers. While
...
This may also occur if we measure outcomes within a single unit (e.g.,
...
even observe) impacts of focal variables, or main effects.
...
The post hoc test results showed  that all groups show a difference. A positive estimated value means that the first listed group has a greater color index than the second group (and vise versa). This is an example of one way to deal with non-independent data.
...
Simply using the provided output (approach 1) and not perform
