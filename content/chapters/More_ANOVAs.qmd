...
$$
\begin{split}
H_O: \mu_\textrm{plant growth, no fertilizer} = \mu_textrm{plant growth, fertilizer}\\
H_O: \mu_\textrm{plant growth, snails removed} = \mu_textrm{plant growth, control snails}= \mu_textrm{plant growth, snails added}\\
...
above-ground biomass (maybe because uncaged plots had little..).
...
```{r}
sdm_model <-lm(Leaf.litter..dry.weight..m2.~Snail.Level * Nitrogen.level, valdez_2023[valdez_2023$Snail.Level != "uncaged",])
plot(sdm_model)
...
```{r}
litter_summary <- summarySE(valdez_2023[valdez_2023$Snail.Level != "uncaged",], measurevar = "Leaf.litter..dry.weight..m2.", groupvars = c("Snail.Level", "Nitrogen.level"))
litter_summary
```

```{r}
litter_summary$Snail.Level <- relevel(litter_summary$Snail.Level, "removal")

ggplot(litter_summary, aes(x=Snail.Level, 
                           y=Leaf.litter..dry.weight..m2.,
                           fill=Nitrogen.level)) +
  geom_col(color="black", position=position_dodge()) +
  geom_errorbar(aes(ymin=Leaf.litter..dry.weight..m2., ymax=Leaf.litter..dry.weight..m2.+ci), position = position_dodge()) +
  labs(main="Grazing impacts depend on nitrogen levels",
       x= "Grazing level",
       y= expression(paste("Leaf litter (" , g^{-1}, m^{-2}, ")")))
```

## Other options

Bootstrapping and permutation tests options may be used for two-way
ANOVAs when assumptions are not met.
