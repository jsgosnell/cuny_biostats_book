...
bird brighter or darker than those on another bird. Regardless of reason (and
...
levels. Under the hood, the linear model approach builds a model matrix
...
Another example we can look at of a paired t test is National No Smoking day, observed in the UK (Waters et al, 1998). There is a possible increase in injuries and workplace accidents on National No Smoking Day due to withdrawal effects people may face by not smoking. Data was collected every year on number of injuries before and on National No Smoking Day for 9 years. We would like to compare the number of injuries before and on No Smoking Day

```{r}
smoking_data <- matrix(c(1987, 1988, 1989, 1990, 1991, 1992, 1993, 1994, 1995, 1996, 516, 610, 581, 586, 554, 632, 479, 583, 445, 532, 540, 620, 599, 639, 607, 603, 519, 560, 515, 556),10, 3, byrow=F)
colnames(smoking_data) <- c("Year", "Injuries_before_No_Smoking_Day", "Injuries_on_No_Smoking_Day")
```

There is variation from year to year. We could standardize, but there are other factors to consider why the variation may occur, such as changes in size of workforce. To account for this variation, we can block the data by year.

For a paired t test, our null hypothesis would be that work related injuries do not change during No Smoking Days (d=0), while our alternative hypothesis would be that there is a change in work related injuries during No Smoking Days (d≠0).

We can calculate the difference (d)

```{r}
smoking_data_x <- data.frame(smoking_data)
smoking_data_x$difference<- smoking_data_x$Injuries_on_No_Smoking_Day - smoking_data_x$Injuries_before_No_Smoking_Day
```

We can use the difference column to run a one sample t test. But here, let's run a paired t test between the before and on No Smoking Day injuries:

```{r}
t.test(smoking_data_x$Injuries_before_No_Smoking_Day, smoking_data_x$Injuries_on_No_Smoking_Day, paired=T)
```
Report t=-2.3605, df=9, p= 0.04257. Reject null hypothesis

In earlier chapters we considered options for one- and two-sample
...
is then evaluated using a binomial test. For two-sample test, the *SIGN.test*
...
There is a significant difference between all of the groups. Special and typical feathers lead to higher color index as compared to odd (because the estimate is positive for these pairs), and typical feathers have lower color index compared to special feathers (negative estimate).

## Other ways to be in multiple groups

In the bird example, one of our categories (bird) was unintentional. We
...
B on some trait. We can fully cross the factors in an experiment (each level of factor A is assigned a level of factor B). Doing so can let us consider the **main effects** (impact) of multiple variables and
...
to one of 3 grazer treatments (ambient number of snails, three times ambient number of snails, and snail removal) and one of 2 nitrogen treatments (ambient nitrogen and nitrogen addition). The experiment design also includes uncaged and cage control groups that did not undergo any treatments.
...
@valdez2023 used t-tests (why?) to consider differences between uncaged and
...
degrees of freedom associated with the residuals and re-partition residuals between the snail and nitrogen groups, which ends up reducing
the MST (mean square of treatment). This is because in 2-way ANOVAs we allocate degrees of
freedom to calculating main effects and interactions. This approach was
likely used in the original manuscript.

Simply using the provided output (approach 1) and not performing
...
## Example: Memory, Age, and Processing

A study by Eysenck (1974) investigated the impact of age and information processing method on memory. The study involved 50 younger and 50 older participants which were randomly assigned into 5 learning groups with individual tasks:

1. Counting group: read through a list of words and count the number of letters in each word (lowest level of processing).
2. Rhyming group: read each word and think of a word that rhymed with it. 
3. Adjective group: give an adjective that could reasonably be used to modify each word in the list.
4. Imagery group: form vivid images of each word (highest level of processing).
5. Intentional group: memorize the words for later recall.

All the subjects went through a list of 27 words 3 times and then were asked to write down all the words they could recall.

The questions posed in this study would be whether memory depends on age and/or processing strategy (main effects), but also whether the effect on processing strategy changes with age (interaction). The data can be plotted for visualization:

```{r}
memory <- read.table("http://www.statsci.org/data/general/eysenck.txt", header = T, stringsAsFactors = T)
str(memory)
library(plyr)
...
On the plot we see that all lines are not parallel which indicates that there may be some interactions between factors. Also, some of the processes are more affected by age (adjective, imagery, and intentional), while others are not (counting and rhyming).

In this example, the null hypotheses would be:

$$
\begin{split}
H_O: \textrm{age does not impact number of words recalled}\\
H_O: \textrm{process does not impact number of words recalled}\\
H_O: \textrm{age and process do not interact to impact number of words recalled}\\
\end{split}
$$

Let's build a linear model considering age, process, and their interaction:
...
The assumptions are met, so we can proceed to making an ANOVA table:
...
First, we look at the interaction term since the main effects cannot be interpreted if interaction is significant. That is the case in the current study (for Age:Process interaction p < 0.05). Next, we can subset the data by age to see the impact of different processing approaches in younger and older group separately by running a one-way ANOVA for each age group. 
...
```
We have a significant impact of processing approach on memory for the younger group. To see which approaches have a significant difference, we can run a post-hoc test:
...
summary(comp_young)
```
ANOVA for the older group:

```{r}
memory_interactions_old <- lm(Words ~ Process, memory[memory$Age == "Older",])
plot(memory_interactions_old)
Anova(memory_interactions_old, type = "III")
```
There is also a significant impact of processing approach on memory for the older group, so we need a post-hoc test again:

```{r}
comp_old <- glht(memory_interactions_old, linfct = mcp(Process = "Tukey"))
summary(comp_old)
```
...
ANOVAs when assumptions are not met, though implementation is
