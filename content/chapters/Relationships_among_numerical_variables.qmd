...
association between the two. This is important to consider before
running your analysis *and* when interpreting outcomes.

For example, since we simply observed flower traits, we could easily
reverse our previous code (plot shown here).
...
**regression** in order to see if, indeed, weight can predict the
variance in miles per gallon and, if so, how much of the variance
(R^2^).
...
# Generate perfectly fitting data
...
Don't only rely on a good R2 value to validate the use of a linear
regression to your data. Always check your assumptions!
