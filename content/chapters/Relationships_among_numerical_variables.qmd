...
We will begin with an example from our favorite iris data. So far we
have considered how species (a categorical independent variable) impacts
measured outcomes (continuous dependent variables). However, we might
also want to consider the relationship between traits (continuous
variables). For example, we might want to know if sepal and petal length
are related. We could plot the data:
...
(continuous) variables; alternatively, one does not. To put this our
null hypothesis framework, we might write:
...
categorical predictor (independent) variables. Unlike examples where we
transformed continuous outcomes (dependent variables) into binomial
data, continuous predictors offers information on order and spacing.
Compared to un-ordered categorical variables (what we've focused on),
the numbers mean something! This allows us (with caution) to estimate
outcomes from un-sampled regions.
...
a **prediction** for any value of sepal length!). This approach,
however, also means we are considering **linear** relationships between
our predictor variables (but note our predictor variables themselves
could be a transformation of an actual measurement, such as a square
root or value cubed!). This is one assumption of linear regression. We
can consider **non-linear** relationships using other techniques (which
we will cover later).
...
could note the potential relationship (now occurring by chance!) between
...
and independent. If assumptions are met consider outcomes.
...
relationship is positive. Note we do not need any post-hoc analyses (why
not?).
...
You may have heard the phrase "correlation does not equal causation"
before, which refers to this "driver-less" model. One famous example of
correlation is that the murder rate tends to rise as ice cream
consumption rates rise. Because we have no reason to believe that an
increase in murder rate would *cause* people to eat more ice cream, nor
do we have a reason to believe that ice cream consumption would cause
people to commit murder, all we really want to look at is the
association between the two. In other words, there is another driver (in
this case, time spent outside in the summer) that can help explain both
factors as opposed to one explaining the other. This is important to
consider before running your analysis *and* when interpreting outcomes.

For an example of an association between variables, since we simply
observed flower traits, we could easily reverse our previous code (plot
shown here).
...
Note that doing so would change the coefficients in our linear model,
but not the direction of the relationship. For this reason, correlation
...
association. This should sound familiar (hold this thought). For a
...
To further explore the difference between regression and correlation,
let's consider another dataset that comes pre-installed with R: mtcars.
More information on the data can be found
[here](https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/mtcars).

```{r}
head(mtcars, 6)
```

Let's consider two continuous variables: miles per gallon (mpg) and car
weight (wt). Because we can make a reasonable hypothesis that car weight
would be a *driver* for miles per gallon, then we should run a
**regression** in order to see if weight has an impact on the variance
in miles per gallon and, if so, how much of the variance (R^2^).

\*Note: For regression, it is important that your driver or independent
variable is always on the x-axis and your outcome variable or dependent
variable is always on the y-axis.

```{r}
cars_regression <- lm(mpg ~ wt, mtcars)
plot(cars_regression)
Anova(cars_regression, Type = "III")
summary(cars_regression)

```

Based on our regression, we see that car weight *does* explain
approximately 75% of the variation in miles per gallon (F~1, 30~ =
91.375, p \< 0.001). Further, the relationship between the two is
negative (as car weight increases, mpg decreases).

```{r}
library(ggplot2)
car_wt_mpg <- ggplot(mtcars, aes(x=wt, y=mpg)) +
  geom_point() +
  labs(title="MPG decreases with car weight",
       x= "Car weight",
       y= "MPG")
car_wt_mpg +                                      
...
However, if we were to look at miles per gallon and 1/4 mile time
(qsec), it's less clear which variable might be "driving" the other, so
we should run a **correlation** to look for an association between the
two.

```{r}
cor.test(~ qsec + mpg, data = mtcars)
```

```{r}
library(ggplot2)
car_qsec_mpg <- ggplot(mtcars, aes(x=qsec, y=mpg)) +
  geom_point() +
  labs(title="Positive association between 1/4 mile time and MPG",
       x= "1/4 mile time",
       y= "MPG")
car_qsec_mpg + stat_smooth(method = "lm", 
              formula = y ~ x, 
              geom = "smooth")

```

We can see that there *is* a significant, positive association between
1/4 mile time and miles per gallon (t = 2.5252, p = 0.01708), but we
cannot use either variable to explain the variation in the other.

\*Note: For correlation, which variable is on which axis is not
important for determining an association. The below code is as correct
as the code above.
...
```{r}
library(ggplot2)
car_mpg_qsec <- ggplot(mtcars, aes(x=mpg, y=qsec)) +
  geom_point() +
  labs(title="Positive association between 1/4 mile time and MPG",
       x= "MPG",
       y= "1/4 mile time")
car_mpg_qsec + stat_smooth(method = "lm", 
              formula = y ~ x, 
              geom = "smooth")
```
...
It's possible to have a case where the R2 value is near to 1 (i.e R2 \~
0.9) in a linear regression, but the fit is not homoscedastic (i.e., the
variance of the residuals is not constant across the range of predictor
values). This can happen in very specific and contrived scenarios where
the variance of the error terms changes systematically with the
independent variable, yet the predicted values still exactly match the
actual values.

Here's an this example:
...
error_variance <- x^1.5 * 0.15
error <- rnorm(100, mean = 0, sd = error_variance)  # Error term
y_observed <- y + error  # Observed y values

# Fit linear model
model <- lm(y_observed ~ x)

# Create a dataframe for plotting
data <- data.frame(x = x, y = y, y_observed = y_observed)

# Plot the data with the fitted line from the model and the actual line
ggplot(data, aes(x = x, y = y_observed)) +
  geom_point(color = "blue", alpha = 0.7) +
  geom_smooth(method = "lm", color = "red") +
  labs(title = "Comparison of Fitted Model and Actual Data", 
       x = "X", 
       y = "Y")

```

Let's now check the assumptions by plotting "model"

```{r}
plot(model)
```

there's a potential "funneling" observed in the residuals vs fitted
plot. You might want to consider another fitting model. However, for
this example, let's proceed to compute R2.

```{r}
summary(model)
```

As you can see, R2 value is near 0.9. However, we might not want to use
this linear model to fit this data due to potential Heteroscedasticity.
Take home message: don't only rely on a good R2 value to validate the
use of a linear regression to your data. Always check your assumptions!
