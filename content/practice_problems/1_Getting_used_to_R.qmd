...
using R. We will do an introduction in class [(download help file)](content/Primer_material/R_primer.R). 
You should also be comfortable using Rstudio and github
[(see help file)](content/Primer_material/Tools_overview.qmd).
...
Is `-1:2` the same as `(-1):2` or `-(1:2)`? INSERT AN R CHUNK HERE
...
How many rows does the **sleep** data set have (hint: `?dim`)? What
...
Produce a plot of how *TotalSleep* differs between primates and
...
Sometimes data doesn't have headers (column names),so you have to
    ...
    "a5_abund", "a6_abund"]. Try it and check your input using the
    `head` command.
