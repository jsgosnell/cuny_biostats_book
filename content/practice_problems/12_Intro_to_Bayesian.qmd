...
Make sure you can describe the main differences between Frequentist,
Likelihood, and Bayesian approaches.

### 2

Review the video we watched in class to make sure you understand the
...
I've shared a script in R that lets you test the Monty Hall idea (like
in the video!). It's the chivers_monty_hall_script from the
[code_examples
...
calculate outcomes under each strategy. \* monty(strat="stay",
print_games=F) \* monty(strat="switch", print_games=F) \*
monty(strat="random", print_games=F)

### 4

Setup the Monty Hall problem as probabilities and convince yourself how
it works. You may want to remember to think about prior and new
information (likelihoods).

### 5

Run the frog analysis (14/18 frogs are right-pawed) assuming an
...
to understand how the prior impacts your posterior.

### 6

Using data from Australian athletes
(https://gksmyth.github.io/ozdasl/oz/ais.html for details), determine if the
...
techniques and a Bayesian approach. For the Bayesian approach, compare
approaches that give the null more and less weight.

Data at

```{r}
sport <- read.table("https://raw.githubusercontent.com/jsgosnell/CUNY-BioStats/refs/heads/master/datasets/ais.txt", header = T, 
                    stringsAsFactors = T)
```
...
Develop a Bayesian model to determine if sepal width (from the iris
