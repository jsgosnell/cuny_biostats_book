...
lecture](../chapters/estimation.qmd){target="_blank"} and the use of ggplots (lots of ggplot2 
example code in the [Summarizing data
lecture](../chapters/summarizing_data.qmd){target="_blank"}
...
First plot how TotalSleep is explained by BrainWt (remember the issues with 
the data).  Use ggplot2 to plot the relationship.

### 2

Next color code each plot point by whether or not its a primate.  In order 
...
Develop a properly-labeled bar graph with error bars to explore how total 
sleep changes with 
...
### 6
To make sure you understand the ideas of sampling, confidence intervals, and
...
For this question you'll need the 
