...
(using R's built-in iris dataset).
...
```{r}
#| label: bar_charts_but_just_points 
...
The cardinal @fig-cardinals data has a longer left tail and is not
symmetric. We call this left- or negatively-skewed data (since it's
...
#| fig-cap: "Example of approximately normal data"
ggplot(virginica, 
       aes(Species,Sepal.Length)) + geom_boxplot(size = 3) +
    labs(title=expression(paste("Sepal lengths of ",italic("I. virginica"))),
       x= "",
       y= "Sepal Length (cm)")+
...
the same information about sepal lengths, for example, using just
points:
...
we can extend histograms and bar charts to them using shapes, colors, or
other features to symbolize the groups. We also need to ensure the
different groups are visible when distributions overlap.

```{r}

```

#data from multiple groups#### #barchart \#### \# #load function from
Rmisc library(Rmisc)
...
geom_col(aes(fill=Species), size = 3) + ylab("Sepal Length
(cm)")+ggtitle("Sepal Length of various iris species")+
...
element_text(face="bold", size=28), axis.text.y = element_text(size=20),
axis.text.x = element_text(size=20), legend.text =element_text(size=20),
legend.title = element_text(size=20, face="bold"), legend.position =
"bottom", plot.title = element_text(hjust = 0.5, face="bold", size=32))+
...
== "setosa","Sepal.Length"\]) plot(seto, add = T, col = "blue",
verticals = T) virg \<- ecdf(iris\[iris\$Species ==
"virginica","Sepal.Length"\]) plot(virg, add = T, col = "orange",
verticals = T)
