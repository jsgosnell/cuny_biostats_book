...
A few notes here. geom_bar and geom_col are very similar commands, but
geom_bar assumes its needs to do something to the data (like count it)
by default, whereas geom_col assumes the data are summarized/ready to
plot as is. The extra argument stat=identity above can usually make
...
with summaries. In those cases, focusing on proportion of outcomes may
be more useful (and will give you the exact same visualization if all
groups were measured the same number of times!). This is sometimes
called a mosaic plot; another way to make them (not shown here) is using
the package *ggmosaic*.
...
iris_both$Population <- c(rep("East",nrow(iris)), rep("West", nrow(iris_new)))
```
...
ggplot(iris_both,aes(x=Species)) +
  ...
  labs(title=expression(paste("Color of ",italic("I. virginica "), "flowers")),
       x= "Species",
       y= "Frequency")+
  scale_fill_manual("legend", values = c("blue" = "blue", "orange" = "orange", "purple" = "purple", "pink"="pink", "yellow"="yellow"))+
  guides(fill = "none")+
