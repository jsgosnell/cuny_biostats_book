...
those results usinga histogram. It's important to realize this is
...
occurred, since those are also useful in considering if the null
hypothesis is valid. Also note those (can) exist on both sides of the
hypothesized value. In this example, our observed proportion was .5.
That's .25 away from the value under the null hypothesis (.75), so we
should all simulations that were . 5 or less or 1 or more. That only
happened
