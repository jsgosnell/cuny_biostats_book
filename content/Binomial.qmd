...
our example). That is known as *binomial* data (so if data can be
divided into two categories, we call it *binomial data*). A special case
of binomial data exists when we only get one organism in our sample
(e.g., one bird, one coin flip). We call this *Bernoulli* data.
...
This distribution allows calculations of how often you would expect *s*
successes for a set number of trials (*n-s*) if a population had a
proportion of *p* for the focal trait. It is based on the multiplication
rule of probability (you can find the probability independent events by
multiplying their respective probabilities) and using combinations to
determine how many ways one can arrange *s* successes in *n* draws
(since order of successes and failures is not important here). It can be
written as
...
Using this distribution we can ask for the probability of obtaining a
signal as more extreme than what we actually observed by chance if the
null hypothesis is true (p-value!). The *dbinom* function in R uses this
distribution.
...
| sample data)**      |                     |                |
+---------------------+---------------------+----------------+
| **Reject H~O~**     | Type I error        | Power (1-      |
|                     | (P\[ɑ\])            | $\beta$ )      |
+---------------------+---------------------+----------------+
...
population distribution in regards to age is the same, we still get
...
+------------+------------+----------------------+--------------------+
|            | *          | **Focused on signals | **Focused on       |
|            | *Two-sided | greater than         | signals less than  |
|            | (          | predicted in null**  | predicted in       |
|            | typical)** |                      | null**             |
+------------+------------+----------------------+--------------------+
| **H~O~**   | p = C      | p \<= C              | p \>= C            |
+------------+------------+----------------------+--------------------+
...
<details><summary> In short, the standard error of *p* is
...
failure (since we only have 2 options). For a single draw (Bernoulli
data), we could simply multiple

</details>

This is actually a bit tricky,and many ways have been proposed to do
this [@subedi2019]. Note, for example, the *binom.confint* function in
the *binom* package gives multiple options. For the function,
